...
import time
from modbus_client_helper import ModbusClientHelper

# =============================================================================
# CONFIGURATION
# =============================================================================
VIDEO_PATH = 'coke_fast.mp4'
YOLO_MODEL_PATH = 'best.pt'  # Your trained YOLOv8s model
...
TARGET_LINE_Y = 696  # Your calibrated target line (from calibration)
...
SAVE_OUTPUT_VIDEO = True  # Set to True to save processed video
OUTPUT_VIDEO_PATH = 'output_processed.mp4'  # Output video file path
OUTPUT_CODEC = 'mp4v'  # Codec: 'mp4v' (MP4), 'XVID' (AVI), 'H264' (MP4)
...
class BottleTracker:
    def __init__(self, model_path, detection_line_x, target_line_y, tolerance):
        ...
        self.plc_client = ModbusClientHelper()
    ...
    def process_bottle(self, frame, bbox, track_id):
        ...
        # Trigger Ejector if Rejected
        if status == 'REJECT_LOW':  # Add REJECT_HIGH if implemented
            print(f"!!! TRIGGERING EJECTOR FOR BOTTLE #{track_id} !!!")
            self.plc_client.trigger_ejector()
...
def main():
    ...
    # Calculate appropriate window size based on DISPLAY_SCALE
    ...
    print(f"Display window: {window_width}x{window_height} (scale: {DISPLAY_SCALE:.2f}x)")
    print(f"Note: YOLO processes at 640px internally, coordinates are in original resolution")
    print("="*70)
    ...
    while True:
        ...
        # Add frame counter if saving video
        if SAVE_OUTPUT_VIDEO:
            total_video_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            progress_text = f"Frame: {total_frames_processed}/{total_video_frames} ({total_frames_processed/max(total_video_frames,1)*100:.1f}%)"
            cv2.putText(vis_frame, progress_text, (10, height - 110),
    ...
    if hasattr(tracker, 'plc_client'):
        tracker.plc_client.close()
